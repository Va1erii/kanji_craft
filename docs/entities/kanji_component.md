...
Tracks whether a component's `logic_hint` has been reviewed by a human. Used as the safety gate for remote sync (see [pipeline.md](../technical/pipeline.md)).
...
| `verification_status` | `VerificationStatus` | Review state for this component. Defaults to `draft`. Only `verified` rows are synced to remote. **Admin-only field** — not present in the client Drift schema (clients only receive `verified` rows after promotion) |
| `ai_confidence` | `float?` | 0.0–1.0 confidence score from the onyomi-matching heuristic. Used to sort the review queue (lowest first). Null if set manually. **Admin-only field** — not present in the client Drift schema |
...
4. New components created by the transformation pipeline must have `verification_status = draft`.
5. Only rows with `verification_status = verified` are eligible for remote sync (see [pipeline.md](../technical/pipeline.md)).
...
- **Bulk auto-verification:** High-confidence matches (`ai_confidence >= 0.95`) can be auto-verified to reduce manual review volume. The threshold is configurable in the Admin Tool.
- **Flagged components:** A `flagged` row is excluded from sync but not deleted — it stays in local production for re-review. An admin can change it to `verified` or `unverified` after correction.
