...
- **Study path switch:** SRS progress is per-item, not per-path — switching paths never resets or loses learned items. When `study_path` changes: (1) prompt the user to pick a `current_level` for the new path, (2) the lesson queue re-filters (e.g. `min_jlpt_level` → `min_grade`), (3) items that already have an SrsCard are excluded from the queue regardless of path. The user may see fewer new items if the paths overlap (e.g. most N5 kanji are also grade 1–2).
